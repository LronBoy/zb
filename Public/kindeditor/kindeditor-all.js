...
KindEditor.plugin('map', function(K) {
	...
	self.clickToolbar(name, function() {
		...
		var iframeHtml = ['<!doctype html><html><head>',
			...
			'	geocoder.geocode({latLng: latlng}, function(results, static) {',
			'		if (static == google.maps.GeocoderStatus.OK) {',
			'			if (results[3]) {',
			...
			'	if (!map) return;',
			'	geocoder.geocode({address : address}, function(results, static) {',
			'		if (static == google.maps.GeocoderStatus.OK) {',
			'			map.setZoom(11);',
...
K.extend(KSWFUpload, {
	init : function(options) {
		...
		var settings = {
			...
			upload_success_handler : function(file, serverData) {
				...
				file.url = data.url;
				K('.ke-img', itemDiv).attr('src', file.url).attr('data-static', file.filestatus).data('data', data);
				K('.ke-static > div', itemDiv).hide();
			}
		};
	...
	getUrlList : function() {
		...
		K('.ke-img', self.bodyDiv).each(function() {
			var img = K(this);
			var status = img.attr('data-static');
	...
	appendFile : function(file) {
		...
		var img = K('<img src="' + file.url + '" class="ke-img" data-static="' + file.filestatus + '" width="80" height="80" alt="' + file.name + '" />');
		photoDiv.append(img);
		K('<span class="ke-delete"></span>').appendTo(photoDiv).click(function() {
			self.removeFile(file.id);
		});
		var statusDiv = K('<div class="ke-static"></div>').appendTo(photoDiv);
		// progressbar
